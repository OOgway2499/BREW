...
export default function CustomerApp() {
  ...
  const [customerName, setCustomerName] = useState(() => localStorage.getItem("btb_name") || "");
  ...
  const add = (item) => {
    setCart(c => { const e = c.find(x => x.id === item.id); return e ? c.map(x => x.id === item.id ? { ...x, qty: x.qty + 1 } : x) : [...c, { ...item, qty: 1 }]; });
  ...
  // Reorder cart functions
  const moveUp = (idx) => {
    if (idx === 0) return;
    setCart(c => { const n = [...c];[n[idx - 1], n[idx]] = [n[idx], n[idx - 1]]; return n; });
  };
  const moveDown = (idx) => {
    if (idx === cart.length - 1) return;
    setCart(c => { const n = [...c];[n[idx], n[idx + 1]] = [n[idx + 1], n[idx]]; return n; });
  };

  async function placeOrder() {
    if (!customerName.trim()) {
      alert("Please let us know your name for the order!");
      return;
    }
    localStorage.setItem("btb_name", customerName.trim());

    setLoading(true);
    const oid = `ORD-${Date.now()}`;
    // Items are saved exactly in the array order defined by the user
    await dbInsertOrder({
      id: oid,
      table: TABLE_NUM,
      ...
      <div style={{ minHeight: "100vh", background: "#fdf8f3", fontFamily: F.body }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "2.5rem 1.5rem", textAlign: "center" }}>
          ...
          <div style={{ background: "#fff", borderRadius: 16, padding: "1.2rem 1.5rem", width: "100%", maxWidth: 360, border: "1px solid rgba(59,31,14,.06)", marginBottom: 16 }}>
            <div style={{ fontSize: ".7rem", color: "#c0a090", letterSpacing: 2, textTransform: "uppercase", marginBottom: 12 }}>Order Sequence</div>
            {cart.map((item, i) => (
              <div key={item.id} style={{ display: "flex", justifyContent: "space-between", fontSize: ".85rem", padding: "8px 0", borderBottom: "1px solid rgba(59,31,14,.05)" }}>
                <span style={{ color: "#5a3a22", display: "flex", alignItems: "center", gap: 8 }}>
                  <span style={{ background: "rgba(59,31,14,.06)", borderRadius: 6, padding: "2px 6px", fontSize: ".65rem", fontWeight: 700, color: "#9a7a5a" }}>#{i + 1}</span>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#fdf8f3", fontFamily: F.body, paddingBottom: 110 }}>
      ...
      <div style={{ background: "linear-gradient(135deg,#3b1f0e,#e8622a)", padding: "2rem 1.5rem", overflow: "hidden", position: "relative" }}>
        ...
        {customerName && <div style={{ fontFamily: F.body, fontSize: ".9rem", fontWeight: 700, color: "#fb923c", marginBottom: 8, letterSpacing: 1, textTransform: "uppercase" }}>Welcome back, {customerName}! ✨</div>}
      ...
      {cartOpen && (
        <div style={{ position: "fixed", inset: 0, zIndex: 200 }}>
          ...
          <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, background: "#fdf8f3", borderRadius: "24px 24px 0 0", animation: "slideUp .35s cubic-bezier(.32,0,.67,0)", maxHeight: "90vh", overflow: "auto", boxShadow: "0 -20px 60px rgba(26,10,4,.25)" }}>
            ...
            {cart.length > 1 && <div style={{ padding: "10px 1.5rem 0", fontSize: ".72rem", color: "#c4501e", fontWeight: 600 }}>↕ Use arrows to set the delivery sequence</div>}
            <div style={{ padding: "8px 1.5rem 10px" }}><span style={{ background: "rgba(232,98,42,.1)", color: "#c4501e", borderRadius: 20, padding: "3px 12px", fontSize: ".75rem", fontWeight: 600 }}>📍 Table {TABLE_NUM}</span></div>
            <div style={{ padding: "0 1.2rem", display: "flex", flexDirection: "column", gap: 8 }}>
              {cart.map((item, i) => (
                <div key={item.id} style={{ display: "flex", alignItems: "center", gap: 10, background: "#fff", borderRadius: 14, padding: "10px", border: "1px solid rgba(59,31,14,.07)" }}>

                  {/* Sequencing Arrows */}
                  {cart.length > 1 && (
                    <div style={{ display: "flex", flexDirection: "column", paddingRight: 4, borderRight: "1px solid rgba(59,31,14,.06)" }}>
                      <button onClick={() => moveUp(i)} disabled={i === 0} style={{ padding: 4, background: "none", border: "none", cursor: i === 0 ? "default" : "pointer", opacity: i === 0 ? 0.2 : 0.7, fontSize: ".9rem" }}>▲</button>
                      <button onClick={() => moveDown(i)} disabled={i === cart.length - 1} style={{ padding: 4, background: "none", border: "none", cursor: i === cart.length - 1 ? "default" : "pointer", opacity: i === cart.length - 1 ? 0.2 : 0.7, fontSize: ".9rem" }}>▼</button>
                    </div>
                  )}
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2 }}>
                      <span style={{ fontWeight: 600, fontSize: ".9rem", color: "#1a0a04", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: "100px" }}>{item.name}</span>
                      <span style={{ background: "rgba(232,98,42,.1)", color: "#c4501e", padding: "2px 6px", borderRadius: 6, fontSize: ".6rem", fontWeight: 700 }}>#{i + 1}</span>
                    </div>
                    <div style={{ fontSize: ".78rem", color: "#c0a090" }}>₹{item.price} each</div>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", background: "rgba(59,31,14,.06)", borderRadius: 50, overflow: "hidden", flexShrink: 0 }}>
                    <button onClick={() => dec(item.id)} style={{ width: 28, height: 28, border: "none", background: "transparent", color: "#5a3a22", cursor: "pointer", fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center" }}>−</button>
                    <span style={{ minWidth: 20, textAlign: "center", fontWeight: 700, color: "#1a0a04", fontSize: ".85rem" }}>{item.qty}</span>
                    <button onClick={() => add(item)} style={{ width: 28, height: 28, border: "none", background: "transparent", color: "#5a3a22", cursor: "pointer", fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center" }}>+</button>
                  </div>
                  <div style={{ fontFamily: F.title, fontWeight: 800, fontSize: "1rem", color: "#1a0a04", minWidth: 38, textAlign: "right", flexShrink: 0 }}>₹{item.price * item.qty}</div>
                  <button onClick={() => rem(item.id)} style={{ background: "transparent", border: "none", color: "rgba(59,31,14,.2)", cursor: "pointer", fontSize: "1rem", paddingLeft: 4 }}>✕</button>
                </div>
              ))}
            </div>
            <div style={{ padding: "1rem 1.2rem 0" }}>
              <input style={{ width: "100%", background: "#fff", border: "1.5px solid rgba(232,98,42,.3)", borderRadius: 12, padding: "14px", color: "#1a0a04", fontFamily: F.title, fontSize: "1rem", outline: "none", marginBottom: 12, fontWeight: 700 }}
                placeholder="👤 Your Name (for the order)" value={customerName} onChange={e => setCustomerName(e.target.value)} />
