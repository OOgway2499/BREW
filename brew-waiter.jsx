                    ...
                    {orders.map(order => {
                      ...
                      return (
                        <div key={order.id} style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "11px 16px", borderBottom: "1px solid rgba(59,31,14,.04)" }}>
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            {order.customer_name && <div style={{ fontSize: ".85rem", fontWeight: 700, color: "#e8622a", marginBottom: 4 }}>👤 {order.customer_name}</div>}
                            <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: order.note ? 8 : 0 }}>
                              {(order.items || []).map((it, i) => (
                                <div key={i} style={{ display: "flex", alignItems: "center", gap: 6 }}>
                                  <span style={{ background: "rgba(59,31,14,.06)", borderRadius: 6, padding: "2px 6px", fontSize: ".6rem", fontWeight: 700, color: "#9a7a5a" }}>#{i + 1}</span>
                                  <span style={{ fontSize: ".82rem", color: "#5a3a22" }}>{it.emoji} {it.name} ×{it.qty}</span>
                                </div>
                              ))}
                            </div>
              ...
              return (
                <div key={order.id} className="card-order"
                  ...
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "13px 16px 10px", borderBottom: "1px solid rgba(59,31,14,.06)" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      {/* Table badge */}
                      <div style={{ background: "#fff", border: `1px solid ${st.border}`, borderRadius: 12, padding: "6px 14px", textAlign: "center", boxShadow: `0 4px 12px ${st.bg}` }}>
                        <div style={{ fontFamily: "'Space Mono',monospace", fontSize: ".5rem", color: st.color, letterSpacing: 1, fontWeight: 700 }}>TABLE</div>
                        <div style={{ fontFamily: "'Fraunces',serif", fontSize: "1.6rem", fontWeight: 800, color: st.color, lineHeight: 1 }}>{tableNum}</div>
                      </div>
                      <div>
                        {order.customer_name ? (
                          <div style={{ fontFamily: "'Fraunces',serif", fontSize: "1.3rem", fontWeight: 800, color: "#1a0a04", letterSpacing: -.5, display: "flex", alignItems: "center", gap: 6 }}>
                            <span style={{ fontSize: "1rem" }}>👤</span> {order.customer_name}
                          </div>
                        ) : (
                          <div style={{ fontFamily: "'Fraunces',serif", fontSize: "1.2rem", fontWeight: 800, color: "#1a0a04", letterSpacing: -.5 }}>Order {String(order.id || "").slice(-4)}</div>
                        )}
                        <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 2 }}>
                          <span style={{ fontFamily: "'Space Mono',monospace", fontSize: ".65rem", color: "rgba(59,31,14,.4)" }}>{timeStr(ts)} · {typeof mins === "number" ? `${mins}m ago` : "just now"}</span>
                  ...
                  <div style={{ padding: "10px 16px", borderBottom: "1px solid rgba(59,31,14,.06)" }}>
                    <div style={{ fontSize: ".65rem", fontFamily: "'Space Mono',monospace", color: "rgba(59,31,14,.4)", letterSpacing: 1, marginBottom: 6 }}>DELIVERY SEQUENCE</div>
                    {(order.items || []).map((item, idx) => (
                      <div key={item.id || idx} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "5px 0", borderBottom: "1px solid rgba(59,31,14,.05)" }}>
                        <span style={{ display: "flex", alignItems: "center", gap: 8, fontSize: ".85rem", color: "#5a3a22" }}>
                          <span style={{ background: "rgba(232,98,42,.1)", color: "#c4501e", padding: "2px 6px", borderRadius: 6, fontSize: ".65rem", fontWeight: 700 }}>#{idx + 1}</span>
                          <span>{item.emoji} {item.name}</span>
                          <span style={{ color: "rgba(59,31,14,.3)", fontFamily: "'Space Mono',monospace", fontSize: ".65rem" }}>×{item.qty}</span>
                        </span>
